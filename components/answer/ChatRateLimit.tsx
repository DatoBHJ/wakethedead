...
interface ChatRateLimitProps {
    rateLimitInfo: RateLimitInfo;
}

const ChatRateLimit: React.FC<ChatRateLimitProps> = ({ rateLimitInfo }) => {
    ...
    useEffect(() => {
        const timer = setInterval(() => {
            const now = Date.now();
            const remaining = Math.max(0, rateLimitInfo.reset - now);
        ...
        >
            <div className="bg-background dark:bg-backgroundsecond p-6 shadow-2xl shadow-black/10 rounded-xl max-w-md mx-auto">
                ...
                <div className="space-y-2 text-sm">
                    {/* <p>Limit: {rateLimitInfo.limit} requests</p>
                    <p>Remaining: {rateLimitInfo.remaining} requests</p> */}
                    <p>Reset in: {timeRemaining}</p>
                </div>
