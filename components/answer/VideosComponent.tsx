...
const VideosComponent: React.FC<VideosComponentProps> = ({ videos, onAddLink }) => {
  ...
  const handleAddLink = (url: string, event?: React.MouseEvent) => {
    if (event) {
  ...
  return (
    <div className="backdrop-blur-xl bg-card-foreground/[3%] dark:bg-card-foreground/5 shadow-lg rounded-lg p-4 mt-4 w-full">
      ...
      {selectedVideo && (
        ...
        >
          <div 
            className="relative w-full max-w-4xl max-h-full bg-black rounded-lg shadow-lg cursor-auto"
          ...
          >
            ...
            <button
              ...
              onClick={(e) => handleAddLink(selectedVideo.link, e)}
            >
